...
class AddressBookFunctions
{
public:
	AddressBookFunctions();
	~AddressBookFunctions();

	int ReadAddressFile(string filename);
	int WriteAddressFile(string filename);
	int DisplayAddressBook();
	int	SearchContact(string name);
	int	DeleteContact(string name);
	int DisplaySortedAddressBook();
	int SetAllMarkedForDeletion(int flag);
	void GetLowerCase(string& inString);
